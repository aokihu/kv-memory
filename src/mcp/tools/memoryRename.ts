...
import { MemoryRenameSchema, type MemoryRenameInput } from "../schemas/memory";
...
export const createMemoryRenameTool = (
  ...
  execute: async (args: MemoryRenameInput) => {
    try {
      ...
      const oldMemory = await kvMemoryService.getMemory(args.old_key);
      if (!oldMemory) {
        return JSON.stringify(
      ...
      const newMemory = await kvMemoryService.getMemory(args.new_key);
      if (newMemory) {
        return JSON.stringify(
          { success: false, message: "key already exists" },
          null,
          2,
        );
      }
