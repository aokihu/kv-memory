...
import { MemoryUpdateSchema, type MemoryUpdateInput } from "../schemas/memory";
...
export const createMemoryUpdateTool = (
  ...
  execute: async (args: MemoryUpdateInput) => {
    try {
      ...
      const existingMemory = await kvMemoryService.getMemory(args.key);
      if (!existingMemory) {
        return JSON.stringify(
      ...
      await kvMemoryService.updateMemory(args.key, args.value, args.links);
