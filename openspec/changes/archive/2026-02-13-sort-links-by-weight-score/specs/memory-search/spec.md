...
### Requirement: Search results with sorted links
When search results include memories with links, the system SHALL sort the Links array by combined score (`link weight × memory score`) in descending order.

#### Scenario: Search returns memory with links
- **WHEN** a search returns a memory that has links to other memories
- **THEN** the Links array in the search result is sorted by `link weight × memory score` in descending order
- **AND** links with higher combined scores appear first

#### Scenario: Search with link sorting parameter
- **WHEN** client calls search API with `sortLinks=true` parameter
- **THEN** all memories in search results have their links sorted by combined score
- **WHEN** client calls search API with `sortLinks=false` parameter
- **THEN** links in search results are returned in their original order or unsorted

#### Scenario: Search performance with link sorting
- **WHEN** searching memories with many links
- **THEN** link sorting does not significantly impact search response time
- **AND** search remains efficient even with large numbers of links per memory