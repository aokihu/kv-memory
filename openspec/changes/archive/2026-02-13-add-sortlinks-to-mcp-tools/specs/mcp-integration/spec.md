...
MCP prompts for memory capture SHALL be updated to reflect removed fields. MCP prompts MAY be updated to mention search capabilities. MCP prompts MAY mention link sorting capabilities.
...
- **AND** schema validates sortLinks parameter as optional boolean or string

#### Scenario: MemoryGet tool schema validation
- **WHEN** MCP validates MemoryGet tool parameters
- **THEN** schema validates key parameter as required string
- **AND** schema validates session parameter as optional string
- **AND** schema validates output_format parameter as optional enum (toon/json)
- **AND** schema validates sortLinks parameter as optional boolean or string

### Requirement: Backward compatibility warnings
MCP tools SHALL provide warnings when old clients attempt to use removed fields. New search tools SHALL not break existing functionality. New sortLinks parameter SHALL not break existing functionality.
...
#### Scenario: SortLinks parameter compatibility
- **WHEN** client does not specify sortLinks parameter
- **THEN** default sorting behavior is applied (links are sorted)
- **AND** existing clients continue to work without modification